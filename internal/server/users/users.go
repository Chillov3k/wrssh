...
func DisconnectUserByConnectionDetails(connectionDetails string) bool {
	lck.RLock()
	var serverConnection ssh.Conn
	for _, user := range users {
		if connection, ok := user.userConnections[connectionDetails]; ok {
	...
	return serverConnection.Close() == nil
}

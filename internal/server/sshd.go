...
func StartSSHServer(sshListener net.Listener, privateKey ssh.Signer, insecure, openproxy bool, dataDir string, timeout int) {
	...
	config := &ssh.ServerConfig{
		...
		PublicKeyCallback: func(conn ssh.ConnMetadata, key ssh.PublicKey) (*ssh.Permissions, error) {
			...
			authorisedKeysPath, pathErr := authorizedUserKeysPath(usersKeysDir, conn.User())
			if pathErr != nil {
				return nil, fmt.Errorf("user (%s) denied login: %s", strconv.QuoteToGraphic(conn.User()), pathErr)
			}
...
func authorizedUserKeysPath(usersKeysDir, username string) (string, error) {
	cleanUser := strings.TrimSpace(filepath.Clean(username))
	switch {
	case cleanUser == "", cleanUser == ".", cleanUser == string(filepath.Separator), cleanUser == "..":
		return "", fmt.Errorf("invalid username")
	case strings.ContainsRune(cleanUser, filepath.Separator):
		return "", fmt.Errorf("invalid username")
	}

	return filepath.Join(usersKeysDir, cleanUser), nil
}
...
func acceptConn(c net.Conn, config *ssh.ServerConfig, timeout int, dataDir string) {
	...
	case "user":
		...
		observers.AdminConnectionState.Notify(observers.AdminSessionState{
			Status:      "connected",
			SessionID:   connectionDetails,
			Username:    sshConn.User(),
			RemoteAddr:  sshConn.RemoteAddr().String(),
			Privilege:   user.PrivilegeString(),
			Timestamp:   time.Now(),
			Source:      "ssh",
			Description: "rssh console session",
		})

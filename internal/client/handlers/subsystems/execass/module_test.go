...
func TestExecassManifestDisabledOnNonWindows(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("non-Windows manifest behavior")
	}
	if manifest := New().Manifest(); !manifest.Disabled {
		t.Fatalf("manifest Disabled = false, want true on non-Windows")
	}
}
